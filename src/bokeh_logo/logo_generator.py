...
def generate_logo(df, logo_type, chain, color, width=None, gene = 'any'):
    if logo_type == 'alignment':
        ...
        logo = draw_seqlogo_from_file(df, color, chain, plot_width=width if width is not None else 100, gene = gene)
